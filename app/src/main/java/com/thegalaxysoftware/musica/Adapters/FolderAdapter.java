...
import com.thegalaxysoftware.musica.Dataloaders.FolderLoader;
...
public class FolderAdapter extends BaseSongAdapter<FolderAdapter.ItemHolder> implements BubbleTextGetter {
    ...
    public void onBindViewHolder(final FolderAdapter.ItemHolder itemHolder, int i) {
        ...
        if (localItem.isDirectory()) {
            ...
            ImageLoader.getInstance().displayImage(MusicaUtils.getAlbumArtUri(song.albumId).toString(),
                    itemHolder.albumArt,
    ...
    private class NavigateTask extends AsyncTask<File, Void, List<File>> {
        ...
        protected void onPostExecute(List<File> files) {
            ...
            mBusy = false;
            PreferencesUtility.getsInstance(mContext).storeLastFolder(mRoot.getPath());
        }
    }
            ...
            if (f.isDirectory() && updateDataSetAsync(f)) {
                ...
                handler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        playAll(mContext, ret, current, -1, MusicaUtils.IdType.NA,
