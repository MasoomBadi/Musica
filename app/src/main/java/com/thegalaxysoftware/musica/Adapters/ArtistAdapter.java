...
import com.thegalaxysoftware.musica.Widgets.BubbleTextGetter;
...
public class ArtistAdapter extends RecyclerView.Adapter<ArtistAdapter.ItemHolder> implements BubbleTextGetter {
    ...
    public ArtistAdapter(Activity context, List<Artist> arraylist) {
        ...
        this.isGrid = PreferencesUtility.getsInstance(mContext).isArtistsInGrid();
    }
    ...
    public void onBindViewHolder(final ItemHolder itemHolder, int position) {
        ...
        String albumNmber = MusicaUtils.makeLabel(mContext, R.plurals.Nalbums, localItem.albumCount);
        String songCount = MusicaUtils.makeLabel(mContext, R.plurals.Nsongs, localItem.songCount);
        itemHolder.albums.setText(MusicaUtils.makeCombinedString(mContext, albumNmber, songCount));
                    ...
                    if (isGrid) {
                        ImageLoader.getInstance().displayImage(artist.mArtwork.get(2).mUrl, itemHolder.artistImage,
                                new DisplayImageOptions.Builder().cacheInMemory(true)
                                        .cacheOnDisk(true)
                                        .showImageOnLoading(R.drawable.defaultmusic)
        ...
        if(MusicaUtils.isLollipop())
            itemHolder.artistImage.setTransitionName("transition_artist_art" + position);
    }
