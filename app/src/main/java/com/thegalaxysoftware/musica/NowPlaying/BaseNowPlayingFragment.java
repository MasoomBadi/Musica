...
import com.thegalaxysoftware.musica.Adapters.BaseQueueAdapter;
...
import com.thegalaxysoftware.musica.Timely.TimelyView;
...
import com.thegalaxysoftware.musica.Utils.SlideTrackSwitcher;
import com.thegalaxysoftware.musica.Widgets.CircularSeekBar;
import com.thegalaxysoftware.musica.Widgets.DividerItemDecoration;
import com.thegalaxysoftware.musica.Widgets.PlayPauseButton;
import com.thegalaxysoftware.musica.Widgets.PlayPauseDrawable;
...
public class BaseNowPlayingFragment extends Fragment implements MusicStateListener {
        ...
        public void run() {
            ...
            if (mProgress != null) {
                ...
                if (elapsedtime != null && getActivity() != null)
                    elapsedtime.setText(MusicaUtils.makeShortTimeString(getActivity(), position / 1000));
            }
            overflowcounter--;
    ...
    public Runnable mUpdateElapsedTime = new Runnable() {
        ...
        public void run() {
            if (getActivity() != null) {
                String time = MusicaUtils.makeShortTimeString(getActivity(), MusicPlayer.position() / 1000);
                if (time.length() < 5) {
    ...
    public void setSongDetails(View view) {
        ...
        if (playPauseFloating != null) {
            playPauseDrawable.setColorFilter(MusicaUtils.getBlackWhiteColor(accentColor), PorterDuff.Mode.MULTIPLY);
    ...
    public void updateSongDetails() {
                ...
                ImageLoader.getInstance().displayImage(MusicaUtils.getAlbumArtUri(MusicPlayer.getCurrentAlbumId()).toString(), albumart,
                        new DisplayImageOptions.Builder().cacheInMemory(true)
                            ...
                            public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                                Bitmap failedBitmap = ImageLoader.getInstance().loadImageSync("drawable://" + R.drawable.defaultmusic);
        ...
        if (songduration != null && getActivity() != null)
            songduration.setText(MusicaUtils.makeShortTimeString(getActivity(), MusicPlayer.duration() / 1000));
    ...
    protected void initGestures(View v) {
        if (PreferencesUtility.getsInstance(v.getContext()).isGesturesEnabled()) {
